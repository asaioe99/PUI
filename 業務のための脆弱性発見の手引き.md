...
-target_method   - persistent modeにおける手法の名前を指定 (原文：name of the method to fuzz in persistent mode.) 
                   For this to work either the method needs to be exported or the symbols
                   for target_module need to be available. そうでない場合、代わりに-target_offsetを使用すること
-target_offset   - Fuzzing対象となるアプリケーション内の目的関数のオフセット
...
また、seedによりFuzzingの結果が受ける影響については、

- https://hexhive.epfl.ch/publications/files/21ISSTA2.pdf

が参考になる。

一般的な指針としては、seedファイルは小さいサイズである方が、総合的にはFuzzingのパフォーマンスを高めるということである。
...
また、詳細は[こちら](https://github.com/asaioe99/PUI/blob/main/%E7%9B%AE%E7%9A%84%E9%96%A2%E6%95%B0%E3%81%AE%E9%81%B8%E5%AE%9A%E4%BE%8B.md)で説明しているので櫃王に応じて参照すると良い。
...
### JWCAD(ver 8.24)の目的関数（まだ作りかけで内容も変更予定。CreateFileの第一引数を調べることに言及する）
...
※```kb```コマンドで引数の一覧を確認して、CreateFile関数の第一引数（ファイル名のポインタ）に対して```du [第一引数]```または```da [第一引数]```を行い、読み込ませているjwwファイルの名前であるかを確認する
...
※以下DeepLによる翻訳案（素晴らしい翻訳精度！）
ビットマップの密度が高いと、ファザーが新しいプログラムの状態を確実に発見するのが難しくなるので、AFL_INST_RATIO=10くらいでバイナリを再コンパイルして、もう一度試してみることをお勧めします (env_variables.txt を参照してください)。

ファザーは高い比率を赤色で表示します。おそらく、極端に毛深いソフトウェア(例えば、v8, perl, ffmpeg)をファジングしない限り、このようなフラグを見ることはないでしょう。

もう一つの行は、バイナリに見られるタプルヒットカウントのばらつきに対処するためのものです。要するに、もしすべてのブランチが常に一定の回数だけ取られるのであれば、私たちが試したすべての入力に対して
この行は "1.00 "と表示されます。他のヒットカウントを発生させると、針は "8.00 "に向かって動き出す。
8.00」（8ビットマップの全ビットがヒット）に向かって動き始めますが、おそらくその極限に達することはないでしょう。

この値は、同じインスツルメンテッドバイナリに依存する複数の異なるファジングジョブのカバレッジを比較するために有用です。
